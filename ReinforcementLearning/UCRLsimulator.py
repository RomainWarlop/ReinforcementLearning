...
class UCRL(object):
    ...
    def __init__(self,alpha,states,s0,actions,transition,data):
        """
        Parameters
        =========
        alpha: float
...
#        print("Policy update time %f seconds" % tot)
    ...
    def run(self,T=1000,gamma=1,n_iter=100,verbose=True):
        ...
        while self.t<T:
            ...
            true_action = self.data.loc[self.t,'version']
            if true_action == a:
                # Observe reward
                r = self.data.loc[self.t,'reward']
                self.reward_sequence.append(r)
                # update      
                self.N.loc[self.state,a]+=1
                self.R.loc[self.state,a]+=r
                self.state = str(self.transition(state_unstr(self.state),a)) # next state
                update_policy = False
...
class linUCRL(object):
    ...
    def __init__(self,alpha,d,states,s0,actions,transition,data):
        """
        Parameters
        =========
        alpha: float
    ...
    def Bellman_a(self,a,V,gamma,str_state,rewards):
        ...
        next_str_state = self.transition(state_unstr(str_state),a)
...
#        t0 = time.time()
        ...
        for str_state in self.states:
            ...
            for a in range(self.cardA):
                ...
                r = self.phi[a].dot(x.T)+self.conf[a]*np.sqrt(x.dot(np.linalg.solve(self.V[a],x.T))) # UCB
                r = r[0][0]
                next_state = self.transition(state_unstr(str_state),a)
                r+=self.gamma*V[next_state]
                r_arms[a] = r
...
#        print("Policy update time %f seconds" % tot)
    ...
    def run(self,T=1000,lbda=0.5,gamma=1,n_iter=100,verbose=False):
        """
        Parameters
        ========
        T: int
        ...
        while self.t<T:
            ...
            # Observe reward
                ...
                a = true_action
                state = state_unstr(self.state)
                # we update nothing else
            
            # update      
            self.state = str(self.transition(state,a)) # next state
            update_policy = False
            for a in self.actions:
