...
phi = {0:[3.4,0.08*m,-0.40*m,0.40*m,-0.15*m,0.01*m],
       ...
       3:[3.5,0.01*m,1.0*m,-1.2*m,0.51*m,-0.08*m],
       4:[2.9,-0.1,-0.1,-0.01,-0.01,-0.01]
      }
arm_label = ['Action','Comedy','Thriller','Drama','Bad']
...
plt.savefig(path+'rewards_function_withDrama.png',bbox_inches='tight')
...
plt.savefig(path+'cumulative_rewards_withDrama.png',bbox_inches='tight')
#plt.show()
...
lgd = plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
#fig.tight_layout()
plt.savefig(path+'pulled_arms_withDrama.png',bbox_extra_artists=(lgd,), bbox_inches='tight')
...
plt.ylim((m,M))    
plt.savefig(path+'all_rewards_withDrama.png',bbox_inches='tight')
...
plt.bar(range(5),
        res.reward,
        color=res.color,width=1)
plt.xticks([0.5,1.5,2.5,3.5,4.5],res.model)
for x in range(5):
    plt.annotate(xy=(x+0.2,m+0.2),s=round(list(res.reward)[x],3),fontsize=20)
plt.ylim((m,M))
plt.savefig(path+'rewards_first_withDrama.png',bbox_inches='tight')
...
for x in range(5):
    plt.annotate(xy=(x+0.2,m+0.2),s=round(list(res.reward)[x],3),fontsize=20)
plt.ylim((m,M))
plt.savefig(path+'rewards_last_withDrama.png',bbox_inches='tight')
