...
mnA = data.loc[str(Astate),'mn_rewardA'] #0.09166072904895288 # computed from user that have a frequency of A of 1
mnB = data.loc[str(Bstate),'mn_rewardB'] # computed from user that have a frequency of A of 0
K = 2
...
#            state = str(next_state_(ast.literal_eval(state),action))
#        
#        rewards_random = np.mean(rewards_random)
#        tmp.append(rewards_random)
...
#plt.savefig(input_path+'byCluster_relative_improvement_d='+str(D)+'.png',bbox_inches='tight')
